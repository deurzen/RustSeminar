...
And so is the \cppin{MouseBindings} type that represents the map between input
...
An instance of this type is stored as a field inside of the \cppin{Model}
instance. Registering the initial bindings happens inside of \cppin{Model}'s
constructor, and can be done rather compactly as follows:

\begin{cppblock}
  Model(/* ... */)
    : mouse_bindings({
      { { Button::Middle, { Ctrl, Shift, Super } },
        { [](Model& model, std::optional<Window> w) {
            if (w) model.set_fullscreen_window(*w);
          }
        }
      },
      // ...
...
% Plus: Rust -> Traits for everything => operator overloading
