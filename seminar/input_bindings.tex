...
Unlike Rust's \rsin{FnMut}, which relies on dynamic dispatch, a
\cppin{std::function} instance will make use of \textit{type erasure}
...
A benefit to \cpp{}'s \cppin{std::function} over Rust's \rsin{FnMut} is
that no virtual method lookup is required at runtime, making it the more
runtime efficient construct. Another benefit to \cppin{std::function} is
