...
one being a lifetype parameter (\rsin{'conn}), and the other being
the type of the structure's \rsin{conn} field (\rsin{Conn}), which
...
constructed at compile-time: one for \rsin{Cycle<Workspace>}, and one for
\rsin{Cycle<Client>}, where \rsin{Workspace} and \rsin{Client} in turn both
implement the \rsin{Identify} and \rsin{Debug} traits (as per the constraint on
\rsin{T}).
...
\begin{cppblock}
  ...
  {
    Logger::log_info("Focusing 0x%#08x.", window);
  }
  // ...
\end{cppblock}
