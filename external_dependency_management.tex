...
Since  the only  external dependency  \wmcpp is  reliant on  is \texttt{libX11},
...
support the \mintinline{cpp}{#pragma once} preprocessor directive, providing the
...
at   compile-time:   one   for  \mintinline{rust}{Cycle<Workspace>},   and   one
for  \mintinline{rust}{Cycle<Client>},  where  \mintinline{rust}{Workspace}  and
\mintinline{rust}{Client} in turn both implement the \mintinline{rust}{Identify}
and    \mintinline{rust}{Debug}   traits    (as    per    the   constraint    on
\mintinline{rust}{T}).
...
consists of  a pointer  to an  instance of  some type  \mintinline{rust}{T} that
implements the set of  traits, as well as a pointer to a  vtable that contains a
function pointer to \mintinline{rust}{T}'s implementation  of each method of the
implemented set  of traits  (and their  supertraits)\cite{therustreference}. Our
\mintinline{rust}{Model} struct's \mintinline{rust}{conn} field  is a pointer to
such a trait object.  Behind the pointer, we can have  any implementation of the
\mintinline{rust}{Connection} trait. Which of  the trait's implementors' methods
are called, is determined at runtime.

Looking back at \wmrs's manifest file, the three binary executable crates in its
...
\wmcpp's \mintinline{cpp}{Connection} abstract class.
...
Performing the call from \wmcpp's \mintinline{cpp}{XConnection} class would look
as follows.
...
\wmcpp's \mintinline{cpp}{Model} class will then \textit{contain} a reference to
\textit{some} implementation of \mintinline{cpp}{Connection}, as follows.
...
\mintinline{rust}{Cycle} trait introduces  a generic type, \mintinline{rust}{T},
that  is   \textit{bound}  by   the  restriction   \mintinline{rust}{Identify  +
Debug},   stipulating   that   whatever   concrete   \mintinline{rust}{T}   gets
passed   in  \textit{must}   implement   both  \mintinline{rust}{Identify}   and
...
our \mintinline{rust}{Cycle}  to work  on the  built-in 32-bit  unsigned integer
type, it would first need to implement \mintinline{rust}{Identify}, which can be
done as follows.
